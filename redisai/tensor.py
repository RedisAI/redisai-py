...
class Tensor(object):
    ...
    def __init__(self, dtype: DType, shape: Sequence[int], value):
        warnings.warn(
            "Tensor APIs are depricated and "
            "will be removed from the future release.",
            UserWarning,
        )
        """
    ...
    def __repr__(self):
        return "<{c.__class__.__name__}(shape={s} type={t}) at 0x{id:x}>".format(
            c=self, s=self.shape, t=self.type, id=id(self)
        )
